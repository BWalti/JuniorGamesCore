namespace JuniorGames.Games
{
    ...
    public class ChainGame : GameBase
    {
        ...
        public ChainGame(IBox box, ChainGameOptions options) : base(box)
        {
        ...
        private async Task CheckOneInput()
        {
            ...
            try
            {
                using (this.Box.LightButtonOnPress())
                {
                    var tokenSource = new CancellationTokenSource(TimeSpan.FromSeconds(5));
                    var result = await this.Box
                        .OnButtonDown
                    ...
                    await this.Box.OnButtonUp.FirstAsync(lbpp => lbpp.Equals(result)).RunAsync(tokenSource.Token);
        ...
        private async Task DisplayChain()
        {
            ...
            {
                await led.SetLight(true, TimeSpan.FromMilliseconds(400));
        ...
        {
            var all = this.Box.LedButtonPinPins.ToList();
        ...
        {
            await this.Box.BlinkAll(1, TimeSpan.FromMilliseconds(500));
        }
