namespace JuniorGames.Games
{
    ...
    public class TwoPlayersGame : GameBase
    {
        ...
        public TwoPlayersGame(IBox box, TwoPlayersGameOptions options) : base(box)
        {
        ...
        private async Task Play()
        {
            ...
            await Task.WhenAll(this.Box.Set(buttonOne, true), this.Box.Set(buttonTwo, true));

            //this.buttonDownSubscription?.Dispose();

            this.buttonDownSubscription = this.Box.OnButtonDown.Subscribe(this.ButtonDown);
        }
        ...
        private async Task ProceedOrGameFinished(int index, IEnumerable<ButtonIdentifier> buttonIdentifiers)
        {
            if (index >= this.randomColors.Count)
            {
                ...
                // yes, we are finished!
                await this.Box.Set(buttonIdentifiers, true);
        ...
        private void Init()
        {
            ...
            var groups = this.Box.LedButtonPinPins
